...
def rollout_once(
    ...
    reference : Optional[Dict[str, object]]
        ...
        reference profile.  Supported options include:
          ...
          - ``{"kind": "piecewise_quad", "tm_frac": 0.5}`` for the
...
if __name__ == "__main__":
    ...
    #%% For rolling with rl
meta = load_meta("demo_sac")
agent = make_agent(
    "sac",
    u_rl_max=meta["u_rl_max"],
    hidden_sizes=tuple(meta.get("hidden_sizes", (32, 32)))
)
agent.load("demo_sac", in_dir=AGENTS_DIR)   # pulls in demo_sac_sac_*.pth/meta
_, logs = rollout_once(plant, nom, task, lqr_w, smc_cfg, agent=agent,
                       cost_cfg=cost_cfg, seed=0, collect_logs=True)
